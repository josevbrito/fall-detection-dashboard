    ...
    <>
      ...
      <div className="flex-1 flex flex-col min-h-screen md:ml-[260px] w-full">
        ...
        <main className="p-gutter md:p-8 w-full max-w-[1440px] mx-auto">
          ...
          {tab === "overview" && <Overview totalDevices={totalDevices} fallTotal={fallTotal} perf={perf} sys={sys} falls={falls} onOpenFalls={() => setTab("falls")} />}
          {tab === "falls" && <Falls devices={devices} selected={selected} setSelected={setSelected} falls={falls} deviceName={deviceName} />}
...
function FallsTable({ falls, onPick, selectedName, compact }) {
  if (!falls?.length) {
    return <div className="text-on-surface-variant text-body-sm px-6 py-6">Nenhuma queda registrada. Rode o load test com --fall-prob &gt; 0.</div>;
  }
  return (
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-outline-variant/50 bg-surface-container-lowest/50">
            ...
            {!compact && <th className="py-3 px-6 text-label-caps text-on-surface-variant uppercase">Impacto (g)</th>}
        ...
        <tbody className="divide-y divide-outline-variant/30 text-body-sm">
          {falls.map((f, i) => (
            ...
            >
              ...
              {!compact && <td className="py-3 px-6 font-mono-data">{f.impact != null ? f.impact.toFixed(2) : f.magnitude != null ? f.magnitude.toFixed(2) : "—"}</td>}
...
/* ============ Visão Geral ============ */
function Overview({ totalDevices, fallTotal, perf, sys, falls, onOpenFalls }) {
  return (
    <>
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-stack-gap md:gap-gutter mb-8">
        ...
        <Kpi label="Em queda agora" icon="warning" value={num(fallTotal)} accent="error">
          <div className="mt-2"><span className="text-label-caps text-error-container">Necessita atenção imediata</span></div>
        </Kpi>
      ...
      <Card className="overflow-hidden">
        <div className="px-6 py-5 border-b border-outline-variant/50 flex justify-between items-center">
          <h2 className="text-title-sm text-on-surface flex items-center gap-2"><span>🚨</span> Quedas recentes</h2>
...
/* ============ Quedas ============ */
function Falls({ devices, selected, setSelected, falls, deviceName }) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-gutter">
      ...
      <Card className="overflow-hidden self-start">
        <div className="px-6 py-5 border-b border-outline-variant/50 flex items-center gap-2">
          ...
          <h2 className="text-title-sm text-on-surface">Quedas detectadas <span className="text-on-surface-variant text-body-sm">({falls.length})</span></h2>
        </div>
